...
function DUnitFrames:TargetFrameSetup()
	...
	if TargetFrameHealthBar then
		hooksecurefunc(TargetFrameHealthBar, "SetStatusBarColor", function(sel, oR, oG, oB)
	...
	C_Timer.After(3, function()
		if TargetFrameHealthBarTextLeft ~= nil and TargetFrameHealthBarTextLeft.hooked == nil then
			...
			hooksecurefunc(TargetFrameHealthBarTextRight, "SetText", function(sel, text)
			...
			hooksecurefunc(TargetFrameHealthBarTextLeft, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameHealthBarText, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameManaBarTextLeft, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameManaBarTextRight, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameManaBarText, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
		...
		if TargetFrameTextureFrameHealthBarTextLeft ~= nil and TargetFrameTextureFrameHealthBarTextLeft.hooked == nil then
			...
			hooksecurefunc(TargetFrameTextureFrameHealthBarTextLeft, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameTextureFrameHealthBarTextRight, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameTextureFrameHealthBarText, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameTextureFrameManaBarTextLeft, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameTextureFrameManaBarTextRight, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
			...
			hooksecurefunc(TargetFrameTextureFrameManaBarText, "SetText", function(sel, text)
				if sel.dufsettext then return end
				sel.dufsettext = true
				DUnitFrames:SetFont(sel)
	...
	-- #TargetFrame
	if TargetFrameHealthBar then
		hooksecurefunc(TargetFrameHealthBar, "SetHeight", function(sel)
		...
		hooksecurefunc(TargetFrameHealthBar, "SetSize", function(sel)
		...
		hooksecurefunc(TargetFrameHealthBar, "SetPoint", function(sel)
	...
	if TargetFrameManaBar then
		hooksecurefunc(TargetFrameManaBar, "SetHeight", function(sel)
			if sel.dufsetheight then return end
			sel.dufsetheight = true
			if DUnitFrames:GetWoWBuild() == "CLASSIC" or DUnitFrames:GetWoWBuild() == "TBC" or DUnitFrames:GetWoWBuild() == "MISTS" then
		...
		hooksecurefunc(TargetFrameManaBar, "SetSize", function(sel)
			if sel.dufsetsize then return end
			sel.dufsetsize = true
			if DUnitFrames:GetWoWBuild() == "CLASSIC" or DUnitFrames:GetWoWBuild() == "TBC" or DUnitFrames:GetWoWBuild() == "MISTS" then
				if 38 - DUnitFrames:HPHeight() > 1 then
		...
		hooksecurefunc(TargetFrameManaBar, "SetPoint", function(sel)
			if sel.dufsetpoint then return end
			sel.dufsetpoint = true
			if DUnitFrames:GetWoWBuild() == "CLASSIC" or DUnitFrames:GetWoWBuild() == "TBC" or DUnitFrames:GetWoWBuild() == "MISTS" then
	...
	if TargetFrameNameBackground then TargetFrameNameBackground:SetParent(DUFHIDDEN) end
	local updateTexture = false
	hooksecurefunc(TargetFrameTextureFrameTexture, "SetTexture", function()
	...
	function DUnitFrames:UpdateTargetTexture()
		...
		if class == "minus" then TargetFrameFlash:SetTexture("") end
		if TargetFrameTextureFrameTexture.spacer == nil then
			...
			hooksecurefunc(TargetFrameTextureFrameTexture.spacer, "SetVertexColor", function(sel, r, g, b, a)
				if sel.dufsetvertexcolor then return end
				sel.dufsetvertexcolor = true
		...
		if TargetFrameHealthBarTextLeft ~= nil then
			...
			if not TargetFrameManaBarTextLeft.hooked then
				...
				hooksecurefunc(TargetFrameManaBarTextLeft, "Show", function(sel, ...) if DUnitFrames:HPHeight() >= 32 then sel:Hide() end end)
			end

			if not TargetFrameManaBarTextRight.hooked then
				TargetFrameManaBarTextRight.hooked = true
				hooksecurefunc(TargetFrameManaBarTextRight, "Show", function(sel, ...) if DUnitFrames:HPHeight() >= 32 then sel:Hide() end end)
			end

			if not TargetFrameManaBarText.hooked then
				TargetFrameManaBarText.hooked = true
				hooksecurefunc(TargetFrameManaBarText, "Show", function(sel, ...) if DUnitFrames:HPHeight() >= 32 then sel:Hide() end end)
			end
		...
		elseif TargetFrameTextureFrameHealthBarTextLeft ~= nil then
			...
			if not TargetFrameTextureFrameManaBarTextLeft.hooked then
				...
				hooksecurefunc(TargetFrameTextureFrameManaBarTextLeft, "Show", function(sel, ...) if DUnitFrames:HPHeight() >= 32 then sel:Hide() end end)
			end

			if not TargetFrameTextureFrameManaBarTextRight.hooked then
				TargetFrameTextureFrameManaBarTextRight.hooked = true
				hooksecurefunc(TargetFrameTextureFrameManaBarTextRight, "Show", function(sel, ...) if DUnitFrames:HPHeight() >= 32 then sel:Hide() end end)
			end

			if not TargetFrameTextureFrameManaBarText.hooked then
				TargetFrameTextureFrameManaBarText.hooked = true
				hooksecurefunc(TargetFrameTextureFrameManaBarText, "Show", function(sel, ...) if DUnitFrames:HPHeight() >= 32 then sel:Hide() end end)
			end
		...
		if CanInspect and GetInspectSpecialization then InspectTargetSpec() end
	end

	local ThreatBorder = nil
	if TargetFrameNumericalThreat ~= nil then ThreatBorder = select(3, TargetFrameNumericalThreat:GetRegions()) end
	function TargetFrame.Think()
		if TargetFrameTextureFrameName then
			TargetFrameTextureFrameName:ClearAllPoints()
			if TargetFrame.buffsOnTop then
				local y = -4
				if DUnitFrames:GetConfig("namemode", "Over Portrait") == "Over Health" then TargetFrameTextureFrameName:SetPoint("TOP", TargetFrameManaBar, "BOTTOM", 0, y) end
			else
				local y = 6
				if DUnitFrames:GetConfig("namemode", "Over Portrait") == "Over Health" then TargetFrameTextureFrameName:SetPoint("BOTTOM", TargetFrameHealthBar, "TOP", 0, y) end
			end

			if DUnitFrames:GetConfig("namemode", "Over Portrait") == "Over Portrait" then TargetFrameTextureFrameName:SetPoint("BOTTOM", TargetFramePortrait, "TOP", 0, 12) end
			if DUnitFrames:GetConfig("namemode", "Over Portrait") == "Inside Health" then
				TargetFrameTextureFrameName:ClearAllPoints()
		...
		if ThreatBorder and ThreatBorder:IsShown() then
			...
			if TargetFrame.DUFThreat == nil then TargetFrame.DUFThreat = TargetFrame:CreateFontString(nil, "ARTWORK") end
	...
	TargetFrame.Think()
	if TargetFrameTextureFrameName then
		hooksecurefunc(TargetFrameTextureFrameName, "SetText", function(sel, text, ...)
			if sel.dufsettext then return end
			sel.dufsettext = true
	...
	if TargetFrameTextureFrameTexture then
		hooksecurefunc(TargetFrameTextureFrameTexture, "SetVertexColor", function(sel, oR, oG, oB)
			if sel.dufsetvertexcolor then return end
			sel.dufsetvertexcolor = true
	...
	if TargetFrameToTTextureFrameTexture then
		hooksecurefunc(TargetFrameToTTextureFrameTexture, "SetVertexColor", function(sel, oR, oG, oB)
			if sel.dufsetvertexcolor then return end
			sel.dufsetvertexcolor = true
